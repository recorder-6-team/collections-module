object dlgBatchAutoNumber: TdlgBatchAutoNumber
  Left = 610
  Top = 482
  BorderIcons = [biSystemMenu]
  BorderStyle = bsSingle
  Caption = 'Batch Auto-Numbering'
  ClientHeight = 183
  ClientWidth = 242
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  Icon.Data = {
    }
  OldCreateOrder = False
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 13
  object lblSelect: TLabel
    Left = 10
    Top = 10
    Width = 220
    Height = 26
    Caption = 'Please select the type used to create the registration numbers:'
    Constraints.MaxWidth = 220
    Constraints.MinWidth = 220
    WordWrap = True
  end
  object btnOK: TButton
    Left = 10
    Top = 150
    Width = 100
    Height = 25
    Caption = 'OK'
    Default = True
    ModalResult = 1
    TabOrder = 3
  end
  object btnCancel: TButton
    Left = 130
    Top = 150
    Width = 100
    Height = 25
    Cancel = True
    Caption = 'Cancel'
    ModalResult = 2
    TabOrder = 4
  end
  object rgNumberType: TRadioGroup
    Left = 10
    Top = 50
    Width = 220
    Height = 90
    Caption = 'Registration number type'
    TabOrder = 5
  end
  object rbPreferred: TRadioButton
    Left = 30
    Top = 110
    Width = 113
    Height = 17
    Caption = '&Preferred'
    TabOrder = 2
    OnClick = rbPreferredClick
  end
  object rbNew: TRadioButton
    Left = 30
    Top = 90
    Width = 113
    Height = 17
    Caption = '&New'
    TabOrder = 1
    OnClick = rbNewClick
  end
  object rbPrevious: TRadioButton
    Left = 30
    Top = 70
    Width = 113
    Height = 17
    Caption = 'Pre&vious'
    Checked = True
    TabOrder = 0
    TabStop = True
    OnClick = rbPreviousClick
  end
end
