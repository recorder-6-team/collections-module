...
type
  ...
  TOSVersion = (
      ...
      wvWinME, wvWinNT, wvWin2k, wvWinXP, wvWin2k3, wvVista, wvSeven);
...
function GetRTFText(AControl: TCustomRichEdit): String;
  ...
  if Win32Platform = VER_PLATFORM_WIN32_NT then
    case Win32MajorVersion of
      ...
      7: case Win32MinorVersion of
           0: Result := wvSeven;   // Future version
         end;
    end
  else
...
{-------------------------------------------------------------------------------
  Extract the RTF from the control and returns it as a string.
}
function GetRTFText(AControl: TCustomRichEdit): String;
var
...
begin
  ...
  try
    ...
    // Also, RTF doesn't use #13#10 to mark a new line, so trim them.
    stream.Position := 0;
    Result :=
        StringReplace(
            StringReplace(stream.ReadString(stream.Size - 1), #13, '', [rfReplaceAll]),
            #10, '', [rfReplaceAll]);
  finally
    stream.Free;
  end;
