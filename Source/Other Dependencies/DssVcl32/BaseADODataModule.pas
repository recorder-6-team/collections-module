...
type
  ...
  protected
    ...
    procedure CancelAsyncCommand(Command: TADOCommand);
    procedure CancelAsyncDataSet(DataSet: TADODataSet);
  ...
  public
    ...
    function RunUserFunction(const AFunctionName: string; AParams: Array of Variant): Variant;
  end;
...
var
  lList: TList;
  lCommandAssistor: TAsyncAssistorNoRecords;
  lFetchAssistor: TAsyncFetchAssistor;
begin
  try
    ...
    try
      ...
      begin
        lCommandAssistor := TAsyncAssistorNoRecords(lList[0]);
        lList.Delete(0);
        CancelAsyncCommand(lCommandAssistor.FCommand);
        lCommandAssistor.Free;
      end;
    finally
    ...
    try
      while lList.Count > 0 do
      begin
        lFetchAssistor := TAsyncFetchAssistor(lList[0]);
        lList.Delete(0);
        CancelAsyncDataSet(lFetchAssistor.FDataset);
        lFetchAssistor.Free;
      end;
    finally
...
procedure TdmBaseADO.CancelAsyncCommands(
  ACallbackProc: TAsyncExecuteCompleteProc);
var
  I: Integer;
  lList: TList;
  lAssistor: TAsyncExecuteAssistor;
  lContinue: Boolean;
begin
  {$IFDEF DEBUG}
  try
  ...
  try
    ...
    begin
      ...
      begin
        ...
        begin
          lAssistor := TAsyncExecuteAssistor(lList[I]);
          if Addr(lAssistor.CallbackProc) = Addr(ACallbackProc) then
          begin
            lContinue := I < lList.Count - 1;
            lList.Delete(I);
            CancelAsyncCommand(lAssistor.FCommand);
            lAssistor.Free;
            // We cannot continue with the 'for' loop because if we are in a
            // single-threaded apartment add-in then calling out to ADO by
            // cancelling the command allows an arbitrary number of ADO
            // completion events to be processed on the current thread, any
            // of which may have modified the list. Instead, we break out of
            // the 'for' and, if necessary (lContinue = True), start again from
            // the beginning.
            Break;
          end;
        end
...
procedure TdmBaseADO.CancelAsyncCommands(
  ACallbackProc: TAsyncFetchCompleteProc);
var
  I: Integer;
  lList: TList;
...
begin
  ...
  lContinue := True;
  try
    // See the comments in CancelAsyncCommands(TAsyncExecuteCompleteProc) for
    // the explanation of the two loops, and the Break from the 'for' loop.
    while lContinue do
    begin
      lContinue := False;
      for I := 0 to lList.Count - 1 do
      begin
        ...
        begin
          lContinue := I < lList.Count - 1;
          lList.Delete(I);
          CancelAsyncDataSet(lAssistor.FDataset);
          lAssistor.Free;
          Break;
        end;
      end;
...
//      if FInCallback then ShowMessage('during callback');
      raise EBaseADODataModuleException.Create(ResStr_ExceptionInMethod +
  ...
  try
    ...
    begin
      ...
      begin
        ...
        begin
          lAssistor := TAsyncRunAssistor(lList[I]);
          if Addr(lAssistor.CallbackProc) = Addr(ACallbackProc) then
          begin
            lContinue := I < lList.Count - 1;
            lList.Delete(I);
...
{-------------------------------------------------------------------------------
  Cancels an asynchronous ADO command.
}
procedure TdmBaseADO.CancelAsyncCommand(Command: TADOCommand);
begin
  Command.CommandObject.Cancel;
end;

{-------------------------------------------------------------------------------
  Cancels an asynchronous ADO fetch.
}
procedure TdmBaseADO.CancelAsyncDataSet(DataSet: TADODataSet);
var
  Command: _Command;
begin
  if not Assigned(DataSet.Recordset) then Exit;

  if not (stFetching in DataSet.RecordsetState) then
  begin
    DataSet.Recordset.Cancel
  end
  else
  begin
    Command := _Command(DataSet.Recordset.ActiveCommand);
    Command.Cancel;
    if Assigned(DataSet.Recordset) then DataSet.Recordset.Cancel;
    TDataSetHack(DataSet).SetState(dsInactive);
    TDataSetHack(DataSet).CloseCursor;
  end;
end;
...
{-------------------------------------------------------------------------------
  Generates a parameterised query for running user functions. Parameter list
  does not contain parameter names as this is not currently required.
}
function TdmBaseADO.RunUserFunction(
...
begin
  ...
  begin
    Close;
    SQL.Clear;
    SQL.Add('select ' + AFunctionName);
    if Length(AParams) > 0 then
    begin

      SQL.Add('(' + ':par0');
      Parameters.ParamByName('par' + IntToStr(0)).Value := AParams[0];
      lCount := 1;
      while lCount < Length(AParams) do
      begin
        SQL.Add(', :par' + IntToStr(lCount));
        Parameters.ParamByName('par' + IntToStr(lCount)).Value := AParams[lCount];
        lCount := lCount + 1;
      end;
      SQL.Add(') as Output');
    end;
    ...
    begin
      if not EOF then
        Result := VarToStr(Fields['Output'].Value);
    end;
  end;
end;
