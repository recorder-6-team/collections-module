...
type
  ...
  protected
    ...
    procedure ResetCustomGridOptions; virtual;
...
{-------------------------------------------------------------------------------
  Prepares the control for having its properties assigned values read from a
  stream.
}
procedure TDBFilterGrid.ReadState(Reader: TReader);
begin
  inherited;
  ResetCustomGridOptions;
end;
...
{-------------------------------------------------------------------------------
  HACK: this type definition makes the protected property TCustomGrid.Options
        visible for use in the ResetCustomGridOptions method.
}
type TCustomGrid = class(Grids.TCustomGrid);

{-------------------------------------------------------------------------------
  Makes changes to the protected Options property inherited from TCustomGrid.
  This is distinct from (and hidden by) the published Options property that is
  inherited from TDBGrid.
}
procedure TDBFilterGrid.ResetCustomGridOptions;
begin
  with TCustomGrid(Self) do Options := Options - [goColMoving];
end;
