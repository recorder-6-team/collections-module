...
type
  ...
  public
    ...
    procedure Init(const ADatabase: TADOConnection; const iTableName: string;
      ...
      iRegDropComp: TRegisterDropComponent; const ASiteID: string);
...
begin
  try
    if FlbExternalRefs.ItemIndex>-1 then
      ...
      ShellFile(FlbExternalRefs.Items[FlbExternalRefs.ItemIndex]);
  except
...
begin
  inherited;
  { Only create the dialog when needed }
  if FdlgOpen = nil then begin
    FdlgOpen := TOpenDialog.Create(Self); // it will free itself! Actually the component will free it
    FdlgOpen.Filter:='All Files (*.*)|*.*';
  end;
  if FdlgOpen.Execute then
    if FlbExternalRefs.Items.IndexOf(FdlgOpen.FileName)=-1 then begin
      lDataItem := TExternalRefItem.CreateNew( FExternalRefList);
      lDataItem.FileName := FdlgOpen.FileName;
