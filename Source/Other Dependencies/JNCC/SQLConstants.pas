...
{$ENDIF}
  ...
  SQL_TAXON_FROM_QUERY_RUCKSACK  =
          'SELECT DISTINCT ITN2.Taxon_List_Item_Key, ITN2.Abbreviation, '+
          '       ITN2.Authority, '''' AS ListName, '+
          '       LSRChild.SearchCode, ' +
          '       dbo.ufn_FormattedSpeciesName( ITN.Actual_Name, ' +
          '                                     ITN.Authority,  ' +
          '                                     ITN3.Authority, ' +
          ...
          'INNER JOIN Taxon_Version TV ON TV.Taxon_Version_Key = TLI.Taxon_Version_Key ' +
          'INNER JOIN Taxon_Rank TR ON TR.Taxon_Rank_Key = TLI.Taxon_Rank_Key ' +
          'INNER JOIN Index_Taxon_Name ITN3 ON ITN3.Taxon_List_Item_Key	= TLI.Preferred_Name ' +
          'INNER JOIN #LastSearchedRucksack LSR ON ITN.Taxon_List_Item_Key = LSR.Taxon_List_Item_Key '+
          'INNER JOIN #LastSearchedRucksack LSRChild ON ITN2.Taxon_List_Item_Key = LSRChild.Taxon_List_Item_Key '+
          'WHERE ITN2.Taxon_List_Version_key = ITN.Taxon_List_Version_key '+
          'AND (ITN2.Actual_Name ' + ST_LIKE_PATTERN +
          ' OR ITN2.Abbreviation ' + ST_LIKE_PATTERN +
          ' OR ITN2.Authority ' + ST_LIKE_PATTERN +
          ' OR (LSR.SearchCode ' + ST_LIKE_PATTERN +
          ' AND ITN2.Taxon_List_Item_Key=ITN.Taxon_List_Item_Key))';

  ST_ITEMNAME = 'ITN.Actual_Name';
  ST_ABBREVIATION = 'ITN.Abbreviation';

  SQL_SUBQUERY_TAXON_GROUPS = 'SELECT Contained_List_Item_Key ' +
                              'FROM Index_Taxon_Group ' +
  ...
  SQL_TAXA_FOR_GROUP_AND_SAMPLE_INTERSECTION =
          ...
          '(Taxon_Occurrence As TOX INNER JOIN '+
          '(Taxon_List_Item As TLI INNER JOIN '+
          '(Index_Taxon_Group As ITG INNER JOIN '+
          'Taxon_Determination As TD ON '+
          'ITG.Contained_List_Item_Key = TD.Taxon_List_Item_Key) ON '+
          'TLI.Taxon_List_Item_Key = ITG.Contained_List_Item_Key) ON '+
          'TOX.Taxon_Occurrence_Key = TD.Taxon_Occurrence_Key) ON '+
          'TV.Taxon_Version_Key = TLI.Taxon_Version_Key) ON '+
          'T.Taxon_Key = TV.Taxon_Key) ON '+
          'SAMPLE.SAMPLE_KEY=TOX.SAMPLE_KEY '+
          'WHERE TD.Preferred = 1 ' +
          'AND SAMPLE.SAMPLE_KEY in (%s) AND ITG.Taxon_List_Item_Key IN (%s)';
...
type
  ...
  TStandardSQL = (ssSurveysForName, ssEventsForName, ssSamplesForName,
                  ...
                  ssLocationsForSample, ssTaxonName, ssCommonName, ssBiotopeName,
