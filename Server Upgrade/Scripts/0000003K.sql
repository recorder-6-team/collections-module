...
SET NOCOUNT ON
	...
	SELECT DISTINCT TLI.Taxon_List_Item_Key,
			...
			END,  
			CASE
				WHEN TLI2.Taxon_Version_Key IS NULL THEN TV1.Taxon_Version_Key
				ELSE TLI2.Taxon_Version_Key
			END,
			CASE 
				WHEN TLI2.Taxon_List_Item_Key IS NULL THEN TLI.PREFERRED_NAME
				ELSE TLI2.Taxon_List_Item_Key
			END,
			TLI.ENTERED_BY,
			TLI.ENTRY_DATE,
			TLI.CHANGED_BY,
	...
	INNER JOIN	Concept				C1		ON	C1.Term_Key					=	TV.Term_Key
											AND	C1.Concept_Group_Key		=	@concept_group_Key
	LEFT JOIN	Taxon_Dictionary_Concept_Mapping 
									TDM2	ON	TDM2.Concept_Key			=	C1.Meaning_Key
	LEFT JOIN	Taxon_List_Item		TLI2	ON	TLI2.Taxon_List_Item_Key	=	TDM2.Taxon_List_Item_Key
